...
void robotAutoMotorMove(struct Robot * robot, int front_centre_sensor, int left_sensor, int right_sensor) {
    ...
    printf("front centre sensor : %d\n", front_centre_sensor);
    
    if (left_sensor > 0 && front_centre_sensor == 0) {
        if (robot->currentSpeed < 2)
            robot->direction = UP;
    } else if (left_sensor > 0 && front_centre_sensor > 0) {
        robot->direction = RIGHT;
    } else if (left_sensor > 0 && front_centre_sensor > 0) {
        robot->direction = RIGHT;
    } else if (left_sensor < 2 && front_centre_sensor == 0) {
